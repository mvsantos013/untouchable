...
public class Level_1 implements Screen, InputProcessor{
    ...
    public Level_1(final Untouchable screen){
        this.screen = screen;
    ...
    public void spawnBallOnScore(){
        switch(timeLeft){
            case(135):
                balls.add(new Ball());
                balls.peek().setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,1,1);
                break;
            case(105):
                balls.add(new Ball());
                balls.peek().setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,1,1);
            ...
            case(55):
                ...
                break;
            case(35):
                balls.get(1).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,1,1);
                break;
            case(30):
                balls.get(2).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,1,1);
                break;
            case(20):
                balls.get(3).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,1,1);
                break;
        }
    }
    ...
    public void gameLoop(float delta){
        ...
        screen.glyphLayout.setText(screen.scoreFont, "Level: 1");
        ...
        if(timeLeft == 0) {
            gameFinished = true;
            gameWon = true;
            screen.settings.putBoolean("level_1_Unlocked", true);
            ...
            if(screen.deathCount == 8){
                if(screen.adsController.isWifiConnected() == 1) {
                    Timer .schedule(new Timer.Task() {
                        @Override
                        public void run() {
                            screen.adsController.showInterstitialAd(screen.runnables.changeToLevel_1());
                            Timer.instance().clear();
                        }
                    }, 1f);
                }
