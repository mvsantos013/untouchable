...
public class Level_2 implements Screen, InputProcessor{
    ...
    public Level_2(final Untouchable screen){
        this.screen = screen;
    ...
    public void spawnBallOnScore(){
        switch(timeLeft){
            case(170):
                balls.add(new Ball());
                balls.peek().setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,2,1);
                break;
            case(155):
                balls.add(new Ball());
                balls.peek().setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,2,1);
            ...
            case(90):
                ...
                break;
            case(70):
                balls.get(1).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,2,1);
                break;
            case(50):
                balls.get(2).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,2,1);
                ...
                break;
            case(25):
                balls.get(5).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,2,1);
                balls.get(0).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,2,1);
                break;
        }
    }
    ...
    public void gameLoop(float delta){
        ...
        screen.glyphLayout.setText(screen.scoreFont, "Level: 2");
        ...
        if(timeLeft == 0) {
            gameFinished = true;
            gameWon = true;
            screen.settings.putBoolean("level_2_Unlocked", true);
            ...
            if(screen.deathCount == 8){
                if(screen.adsController.isWifiConnected() == 1) {
                    Timer .schedule(new Timer.Task() {
                        @Override
                        public void run() {
                            screen.adsController.showInterstitialAd(screen.runnables.changeToLevel_2());
                            Timer.instance().clear();
                        }
                    }, 1f);
                }
