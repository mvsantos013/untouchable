...
public class Level_5 implements Screen, InputProcessor{
    ...
    private Sprite objectBall_1, objectBall_2, objectBall_3, objectBall_4, objectBall_5, objectBall_6;
    ...
    public Level_5(final Untouchable screen){
        this.screen = screen;
    ...
    public void spawnBallOnScore(){
        switch(timeLeft){
            ...
            case(125):
                ...
                balls.get(1).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                break;
            case(105):
                balls.get(2).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                break;
            case(95):
                balls.get(3).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                break;
            case(70):
                balls.get(4).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                balls.get(5).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                balls.get(6).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                break;
            case(45):
                balls.get(0).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                break;
    ...
    public void gameLoop(float delta){
        ...
        screen.glyphLayout.setText(screen.scoreFont, "Level: 5");
        ...
        if(timeLeft == 0) {
            gameFinished = true;
            gameWon = true;
            screen.settings.putBoolean("level_5_Unlocked", true);
            ...
            if(screen.deathCount == 8){
                if(screen.adsController.isWifiConnected() == 1) {
                    Timer .schedule(new Timer.Task() {
                        @Override
                        public void run() {
                            screen.adsController.showInterstitialAd(screen.runnables.changeToLevel_5());
                            Timer.instance().clear();
                        }
                    }, 1f);
                }
    ...
    public boolean objectBallsCollided(){
        return((Math.pow(posPlayer.x - (objectBall_1.getX() + 0.5f*objectBall_1.getWidth()), 2) + Math.pow(posPlayer.y - (objectBall_1.getY() + 0.5f*objectBall_1.getHeight()), 2)) < Math.pow(2*playerRadius, 2) ||
                (Math.pow(posPlayer.x - (objectBall_2.getX() + 0.5f*objectBall_2.getWidth()), 2) + Math.pow(posPlayer.y - (objectBall_2.getY() + 0.5f*objectBall_2.getHeight()), 2)) < Math.pow(2*playerRadius, 2) ||
                (Math.pow(posPlayer.x - (objectBall_3.getX() + 0.5f*objectBall_1.getWidth()), 2) + Math.pow(posPlayer.y - (objectBall_3.getY() + 0.5f*objectBall_1.getHeight()), 2)) < Math.pow(2*playerRadius, 2) ||
