...
public class Level_6 implements Screen, InputProcessor{
    ...
    public Level_6(final Untouchable screen){
        this.screen = screen;
    ...
    public void spawnBallOnScore(){
        switch(timeLeft){
            ...
            case(40):
                ...
                balls.get(7).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                break;
            case(20):
                balls.get(8).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                balls.get(0).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                balls.get(1).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5,1);
                balls.get(2).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,5 ,1);
                break;
    ...
    public void gameLoop(float delta){
        ...
        screen.glyphLayout.setText(screen.scoreFont, "Level: 6");
        ...
        if(timeLeft == 0) {
            gameFinished = true;
            gameWon = true;
            screen.settings.putBoolean("level_6_Unlocked", true);
            ...
            if(screen.deathCount == 8){
                if(screen.adsController.isWifiConnected() == 1) {
                    Timer .schedule(new Timer.Task() {
                        @Override
                        public void run() {
                            screen.adsController.showInterstitialAd(screen.runnables.changeToLevel_6());
                            Timer.instance().clear();
                        }
                    }, 1f);
                }
