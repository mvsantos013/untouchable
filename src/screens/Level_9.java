...
public class Level_9 implements Screen, InputProcessor{
    ...
    public Level_9(final Untouchable screen){
        this.screen = screen;
    ...
    public void spawnBallOnScore(){
        switch(timeLeft){
            ...
            case(80):
                ...
                break;
            case(70):
                balls.get(6).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,6,1);
                break;
            case(60):
                balls.get(7).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,6,1);
                break;
            case(50):
                balls.get(8).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,6,1);
                break;
            case(40):
                balls.get(9).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,6,1);
                break;
            case(30):
                balls.get(10).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,6,1);
                break;
            case(20):
                balls.get(11).setBallAttributes(screen.camera.viewportWidth, screen.camera.viewportHeight, posPlayer,6,1);
                break;
        }
    }
    ...
    public void gameLoop(float delta){
        ...
        screen.glyphLayout.setText(screen.scoreFont, "Level: 9");
        ...
        if(timeLeft == 0) {
            gameFinished = true;
            gameWon = true;
            screen.settings.putBoolean("level_9_Unlocked", true);
            ...
            if(screen.deathCount == 8){
                if(screen.adsController.isWifiConnected() == 1) {
                    Timer .schedule(new Timer.Task() {
                        @Override
                        public void run() {
                            screen.adsController.showInterstitialAd(screen.runnables.changeToLevel_9());
                            Timer.instance().clear();
                        }
                    }, 1f);
                }
